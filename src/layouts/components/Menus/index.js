  ...
  renderMenu(data = [], pathtitles = []) {
    ...
    return rows.map(row => {
      ...
      if (children && children.length > 0) {
        const subMenu = this.renderMenu(children, pathtitles.concat(name));
        return (
          <SubMenu
            key={key}
            text={name}
            title={
              <span>
                {' '}
                [Icon] {mode === 'inline' ? <span>{name}</span> : null}
              </span>
            }
          >
            {subMenu}
        ...
        if (link === '' && href) {
          return (
            <Item key={href.slice(-5)} text={name}>
...
export default connect(({ menu: { menusData } }) => ({ menusData }))(
  Consumer(MainMenu),
);

MainMenu.propTypes = {
  menusData: PropTypes.arrayOf(function(
    propValue,
    key,
    componentName,
    location,
